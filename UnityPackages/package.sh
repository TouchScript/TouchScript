...
for i in $(ls -d */); do 
	...
	if [[ $name != _* ]]; then
		...
		"$unityPath" -batchmode -logFile -projectPath "$folder/$name" -exportPackage $toExport $package -quit
		"$unityPath" -batchmode -logFile -projectPath "$folder/$name" -exportPackage $toExport Assets/TouchScript/Examples "$folder/_AssetStore/Assets/TouchScript/Examples/$name.Examples.unitypackage" -quit
		if [ ! -f $package ]; then
...
"$unityPath" -batchmode -logFile -projectPath "$folder/_AssetStore" -exportPackage Assets/TouchScript "$folder/TouchScript.unitypackage" -quit