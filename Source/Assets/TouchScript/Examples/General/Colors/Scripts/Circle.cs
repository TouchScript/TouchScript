...
namespace TouchScript.Examples.Colors
{
	public class Circle : MonoBehaviour 
	{
		...
		private void OnTriggerEnter2D(Collider2D other)
		{
			...
			var scale = Mathf.Sqrt(otherCircle.transform.localScale.x * otherCircle.transform.localScale.x + transform.localScale.x * transform.localScale.x);
			var color = Color.Lerp(renderer.sharedMaterial.color, otherColor, .5f);
